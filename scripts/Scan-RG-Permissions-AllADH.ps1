...
function Ensure-Dir([string]$p){ if([string]::IsNullOrWhiteSpace($p)){ $p=Join-Path (Get-Location) 'rg-perms-out' } if(-not(Test-Path $p)){ New-Item -ItemType Directory -Path $p -Force | Out-Null } $p }
...
function Load-Expected($p){
  ...
  foreach($need in 'resourcegroupname','roledefinitionname','adgroupname'){ if(-not $map.ContainsKey($need)){ throw "CSV '$p' missing column like '$need'" } }
...
function CustodianFromSub([string]$n){ if($n -match '(?i)ADH([A-Za-z0-9_-]+)'){ $Matches[1] } else { $null } }
...
foreach($sub in $subs){
  ...
  foreach($e in $expected){
    ...
    $rgObj = $null; $key = ($rgName ? $rgName.ToLowerInvariant() : '')
